...
t_7 = {x: len(x) for x in string.split()}
...
t_8 = {x: string.count(x) for x in string if x != ' ' and ':'}

# 9. На входе список чисел, получить список квадратов этих чисел / use map

t_9 = list(map(lambda x: x ** 2, [x for x in range(1, 15)]))
...
t_10 = {(x, y): ((x ** 2 + y ** 2) ** (0.5)) for x, y in coordinates if y == 5 * x - 2}

# 11. Возвести в квадрат все четные числа от 2 до 27. На выходе список.

t_11 = [n ** 2 for n in range(2, 28) if n % 2 == 0]
...
t_12 = max([(x**2 + y**2)**0.5 for x, y in coor if x >=0 and y >= 0])
...
t_13 = [[(x+y,x-y)] for x, y in zip(nums_first, nums_second)]

# 14. На входе список строк из чисел, например, ['43141', '32441', '431', '4154', '43121'].
# Найти четные квадраты этих чисел. Ответ записать снова в список из строк,
# то есть сформировать обратно список строк, но уже отфильтровать все четные квадраты.

digits = ['43141', '32441', '431', '4154', '43121']

t_14 = list(filter(lambda i: ((int(i) ** 2) % 2 == 0), digits))
